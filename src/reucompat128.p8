...
%option merge
    ...
    asmsub  x16jsrfar() {
        %asm {{
            ...
            ; check if target bank is below 128 (above 128 is REU)
            lda  $02                ; target bank
            bpl  +                  ; bit 7 not set, not negative, normal path
            jmp  reujsrfar          ; finish REU based call.
            ;lda  $fa                ; if I wanted to match what is normally in A
...
+           jsr c128.JSRFAR        ; call kernal's jsrfar routine
    ...
    asmsub reujsrfar() {
        %asm {{
            ; now perform REU based JSRFAR.
            lda  $ff00              ; get current MMU Config Register
            sta  c128.mmucr         ; save for later restoration

            ; swap REU to target bank -128 
            lda  $02                ; target bank
            and  #%01111111         ; turn off high bit (subtract 128)
            sta  p8b_reu.p8s_bank.p8v_banknum 
            jsr  p8b_reu.p8s_bank   ; swap bank

            ; lie about target bank to kernal
            lda  #0                 ; all ram bank 0
            sta  $02                ; target bank

            ; do actual call.
+           jsr c128.JSRFAR        ; call kernal's jsrfar routine

            lda  c128.mmucr         ; load stashed MMU value
            sta  $ff00              ; restore prior MMU value
